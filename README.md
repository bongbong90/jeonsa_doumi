...
- Colab Large-v3 전사(`faster-whisper` 서버 연동)
- 완료 파일 자동 스킵(결과물 무결성 기준)
- Dashboard 통계(누적/오늘/속도/최근 완료)
- Folders 탭 파일 필터/미리보기
- 트레이 아이콘 + 커스텀 토스트 알림 + 전체 완료 후 종료 옵션
...
3. 노트북 셀 1~4를 순서대로 실행합니다.
4. Cloudflare tunnel URL(`https://...trycloudflare.com`)을 앱 `Colab URL`에 입력합니다.
5. `연결 확인`으로 `/health` 응답 확인 후 전사를 시작합니다.
...
4. 필요 시 선택 파일 이동
5. 전사 방식 선택(로컬/Colab)
6. 전사 시작(전체 또는 이동 후 시작)
7. 진행률/로그/알림 확인
8. 결과 파일 확인(TXT/JSON/SRT)

## 8. 출력 파일 구조
입력 MP3 기준으로 같은 폴더에 결과 파일 3종을 생성합니다.
...
파일명 끝의 페이지 표기 패턴(`(p.123)`, `(p.12~34)` 등)은 정리된 이름을 결과 파일 basename으로 사용합니다.
