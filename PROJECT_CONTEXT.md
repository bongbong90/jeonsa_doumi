...
Colab chunk timeout resilience
- chunk 기본 분할 단위는 300초이다.
- chunk 전송 중 HTTP 502/503/504/524 또는 timeout 계열 오류가 발생하면 해당 chunk만 재시도한다.
- 최대 재시도는 2회이며, 총 시도는 최초 1회 포함 최대 3회이다.
- 재시도 대기는 10초, 20초 순서로 적용한다.
- 이미 성공한 chunk는 다시 처리하지 않고, 실패한 chunk만 재시도한다.
- HTTP 400/401/403/404 등 non-retry 오류는 재시도하지 않고 즉시 실패 처리한다.
- 사용자 중지 요청이 들어오면 재시도하지 않고 중단 흐름을 따른다.
